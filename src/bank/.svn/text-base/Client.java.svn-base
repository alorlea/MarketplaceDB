...
public class Client {    
	...
	private static final String DEFAULT_BANK_NAME = "rmi://localhost/bank";
	IAccount account;
	IBank bankobj;
	...
	public Client(String bankName) {
		...
		try {
			bankobj = (IBank)Naming.lookup(bankname);
		} catch (Exception e) {
	...
	void execute(Command command) throws RemoteException, RejectedException {
		...
		// all further commands require a IAccount reference
		IAccount acc = bankobj.getAccount(userName);
		if (acc == null) {
